...
def s3_connector(bucket: str, key: str, file_type: str = "csv"):
    """
    key = single file  → "shiv/sales.csv"
    key = folder/prefix → "shiv/"  (sab files process hongi)
    """
    try:
        s3 = boto3.client(
            "s3",
        ...
        # ── Folder/Prefix hai ya single file ─────────────
        if key.endswith("/") or "." not in key.split("/")[-1]:
            ...
            # Sab files ek saath merge karke return karo
